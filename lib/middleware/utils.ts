...
export const parse = (req: NextRequest) => {
	...
	if (domain === "saroh.localhost:8888" || domain === "staging.saroh.io") {
		// for local development and staging environments
		domain = "saroh.io";
	}

	// path is the path of the URL (e.g. saroh.io/stats/github -> /stats/github)
	let path = req.nextUrl.pathname;

	// special case for saroh.io/___saroh_check/ (for checking if saroh.io links are working)
	if (path.startsWith("/___saroh_check/")) {
		domain = "saroh.io";
		path = path.replace("/___saroh_check/", "/");
	}
	...
	const key = decodeURIComponent(path.split("/")[1]); // key is the first part of the path (e.g. saroh.io/stats/github -> stats)
	const fullKey = decodeURIComponent(path.slice(1)); // fullKey is the full path without the first slash (to account for multi-level subpaths, e.g. saroh.io/github/repo -> github/repo)

	return { domain, path, fullPath, key, fullKey };
};

export const getFinalUrl = (target: string, { req }: { req: NextRequest }) => {
	// query is the query string (e.g. saroh.io/github?utm_source=twitter -> ?utm_source=twitter)
